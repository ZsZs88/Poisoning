...
for i in [20, 30, 40, 50]:
    df = pd.read_csv("..\\{}iterative_by_benignnumber{}_preds.csv".format(filenames.dir_aggregated, i), index_col=False)[["index", "BasePred", "5%", "10%", "20%"]]
    print("\n\n{}".format(i))
    print("\n\nDataset Statistics")
    print(df.describe())
    myquery(df, '`5%` >= 0.5')
...
for i in [20, 30, 40, 50]:
    df = \
    pd.read_csv("..\\{}iterative_by_benignnumber{}_accs.csv".format(filenames.dir_aggregated, i), index_col=False)[
        ["index", "5%", "10%", "20%"]]
    print("\n\n{}".format(i))
    print("\n\nDataset Statistics")
    print(df.describe())
...
for i in [5, 10, 20]:
    df = pd.read_csv("..\\{}iterative_by_percent{}_preds.csv".format(filenames.dir_aggregated, i), index_col=False)[
        ["index", "BasePred", "20", "30", "40", "50"]]
    myplot(df, "eval_iterative_by_percent{}_preds".format(i), acc=False)
    # input()

for i in [5, 10, 20]:
    df = pd.read_csv("..\\{}iterative_by_percent{}_accs.csv".format(filenames.dir_aggregated, i), index_col=False)[
        ["index", "20", "30", "40", "50"]]
    myplot(df, "eval_iterative_by_percent{}_accs".format(i), acc=True)
    # input()
